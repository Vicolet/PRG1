...
bool prime(Uint nombrePremier) {
   ...
   for (uint64_t i = 0; i != 10; i++) {
      auto gen_int1_3 = std::bind(std::uniform_int_distribution<uint64_t>(1, uint64_t (nombrePremier - 1)), std::mt19937(987));
      uint64_t nombreAleatoire = gen_int1_3();
      Uint alea(nombreAleatoire);

      if (exponentiationModulaire(alea, nombrePremier - 1, nombrePremier) != 1) {
         return false;
      }
